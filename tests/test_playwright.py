...
BASE_URL = os.environ["MINITWIT_URL"]
...
def sign_in_using_playwrght(page: Page, useTestUser = True):
    ...
    page.locator("input[name='username']").fill(test_username if useTestUser else username)
    page.locator("input[name='password']").fill(test_password if useTestUser else password)
    page.locator("input[type='submit']").click()
...
execute_postgres_query(f"do $$ begin IF NOT EXISTS (SELECT 1 FROM users WHERE username = '{test_username}') THEN INSERT INTO users (username, email, pw_hash) VALUES ('{test_username}', '{test_email}', '{test_password}'); END IF; end $$;")
...
def test_register_works_redirects_to_sign_in(page: Page):
    ...
    does_user_exist_in_db = fetch_postgres_query(f"SELECT count(*) FROM users WHERE username = '{username}'")
